...
describe("BalancedSearchTree", () => {
    describe("Insert / Search", () => {
        ...
        it("Should remain balanced", () => {
            const count = 1000;
            let tree = new BalancedSearchTree_1.BalancedSearchTree((a, b) => a - b);
            let vals = new Array(count).fill(0).map((_, i) => i);
            vals.forEach(val => tree.insert(val));
            expect(tree.root.height).toBeLessThan(Math.log2(vals.length) * 2);
            tree = new BalancedSearchTree_1.BalancedSearchTree((a, b) => a - b);
            ...
            for (const del of ["incr", "decr"])
                for (const type of ["module", "half"]) {
                    it(`Should remain balanced (ins;${ins} del;${del} type;${type})`, () => {
                        ...
                        vals.forEach(val => tree.insert(val));
                        let dels = type == "modulo"
                            ? vals.filter((v, i) => i % 3 == 0)
                            : vals.slice(vals.length / 2);
                        if (ins != del)
                            dels.reverse();
                        dels.forEach(val => tree.delete(val));
                        expect(tree.root.height).toBeLessThan(Math.log2(vals.length - dels.length) * 2);
                    });
                }
        it("Should handle duplicates", () => {
            const tree = new BalancedSearchTree_1.BalancedSearchTree((a, b) => a - b);
    ...
    describe("FindNext", () => {
        it("Should correctly obtain the next item if available", () => {
            const tree = new BalancedSearchTree_1.BalancedSearchTree((a, b) => a - b);
            const vals = (0, shuffle_1.shuffle)([1, 2, 3, 4, 8, 10, 14, 20, 25, 39]);
            vals.forEach(val => tree.insert(val));
    ...
    describe("FindRange", () => {
        it("Should correctly find any items within a specified range", () => {
            ...
            expect(tree.findRange(a => 15 - a, a => 22 - a)).toEqual([20]);
            expect(tree.findRange(-1, 90)).toEqual([1, 2, 3, 4, 8, 10, 14, 20, 25, 39]);
        });
        it("Should report all duplicates in a range", () => {
            const tree = new BalancedSearchTree_1.BalancedSearchTree((a, b) => a - b);
            const vals = (0, shuffle_1.shuffle)([1, 2, 3, 4, 4, 8, 10, 10, 10, 14, 14, 20, 25, 39]);
            vals.forEach(val => tree.insert(val));
